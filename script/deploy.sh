...
for name in "NGINX" "MERCHANT-HOME" "MERCHANT-SDK-PG-TOKENIZATION" "MERCHANT-SDK-PG-CHARGE" "MERCHANT-SDK-CARD" "INGENICO" "ACCOUNT"
do
  echo "push $name"
...
envsubst '$NGINX_IMAGE_NAME,$MERCHANT_HOME_IMAGE_NAME,$MERCHANT_SDK_PG_TOKENIZATION_IMAGE_NAME,$MERCHANT_SDK_PG_CHARGE_IMAGE_NAME,$MERCHANT_SDK_CARD_IMAGE_NAME,$INGENICO_IMAGE_NAME,$ACCOUNT_IMAGE_NAME,$CONTAINER_SERVICE_NAME,$MERCHANT_SERVER_URL,$ACCOUNT_SERVER_URL,$ACCOUNT_CLIENT_ID,$ACCOUNT_CLIENT_SECRET,$OF_AUTH_URL,$OF_ISSUER_URL,$OF_API_URL,$BASIC_AUTH_CREDENTIALS,$SESSION_SECRET,$MERCHANT_PAT_LINK_IMAGE_NAME' < ./script/deploy-template.json > "containers.json"
